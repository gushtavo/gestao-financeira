# 📊 Projeto: Análise Financeira Empresarial

## 1️⃣ PROBLEMA 

🎯 Contexto

Uma empresa de médio porte apresentou crescimento consistente de receita ao longo de 36 meses.
Apesar disso, a diretoria percebe que:

- O saldo de caixa não cresce proporcionalmente

- A margem operacional oscila

- Existem meses de pressão financeira

A gestão precisa entender:

- O crescimento atual é saudável e sustentável no médio prazo?

## ❓ Perguntas de Negócio

Após análise das views e KPIs, foram identificados padrões relevantes:

## A despesa está crescendo mais rápido que a receita?

<img width="1068" height="402" alt="Captura de tela 2026-02-25 202709" src="https://github.com/user-attachments/assets/2dd6ec5c-13e0-4e64-9ffd-23eebe200154" />

📈 Receita vs Despesa

  - Receita apresenta crescimento gradual ao longo dos meses.
  
  - Em determinados períodos, a despesa cresce em ritmo superior.
  
  - Isso impacta diretamente a margem operacional.

🔎 Insight:

- Crescimento de receita não garante aumento de lucro se a estrutura de custos não for controlada.


## Quais centros de custo pressionam a margem?
  
<img width="685" height="260" alt="Captura de tela 2026-02-25 202756" src="https://github.com/user-attachments/assets/7f0a597a-efa7-4168-b853-90856dcfe586" />

<img width="1085" height="269" alt="Captura de tela 2026-02-25 202826" src="https://github.com/user-attachments/assets/065bb417-82bd-4c40-8e05-8e47481e89f5" />

  🏢 Centros de Custo

  - Operações representa a maior fatia de despesa.
  
  - Alguns centros apresentam crescimento percentual acima da média.

O setor de operacões está sobrecarregados, aumentando consideravelmente as depesas na parte operacional, precisamos melhorar a eficiência dessa operação dividindo as tarefas de forma adequada para cada setor tornando o trabalho mais fluído e compatível para todos, e consequentemente ajustando as despesas.

🔎 Insight:

- Existe concentração de custos em poucos centros, indicando possível ineficiência operacional.


## O tempo de recebimento (DSO) é eficiente?
<img width="1013" height="243" alt="Captura de tela 2026-02-25 203123" src="https://github.com/user-attachments/assets/8fb60e96-e20a-437a-838f-2a82ba99ad98" />

  ⏳ Eficiência Financeira

  - DSO varia ao longo do tempo.
  
O prazo de recebimento é de 17 dias e olhando pro gráfico a média de dias não está estável e muito vezes crescente isso indica piora na cobrança e risco de caixa, a sugestão é melhorar a forma de cobrança utilizando avisos de cobrança dois dias antes e no dia do vencimento, isso vai ajudar identificar quem são os melhores pagadores e os que tem atrasos recorrentes.

## O tempo de pagamento (DPO) está sendo bem utilizado?
  <img width="1004" height="238" alt="Captura de tela 2026-02-25 203143" src="https://github.com/user-attachments/assets/442a2d2b-59ef-4dfc-9ede-801bcfddbad9" />

⏳ Eficiência Financeira

- DPO nem sempre compensa o tempo de recebimento.

levando em consideração o prazo dos fornecedores que é de 25 dias para o pagamento, a empresa está pagando cedo demais e isso está sufacando o caixa e acaba ficando sem uma visão adequada da receita que estamos tendo durante os meses, a média ta muito próxima da media de dias de recebimento esse fluxo de caixa não é saudável.


🔎 Insight:

- A empresa pode melhorar sua gestão de capital de giro ajustando prazos de recebimento e pagamento.

## 3️⃣ SOLUÇÃO
🧠 Abordagem Analítica

  - Foi desenvolvido um modelo analítico com:
  
  - Modelo estrela no Power BI
  
  - Views SQL para consolidação
  
  - KPIs financeiros padronizados
  
  - Dashboard executivo + páginas analíticas

📊 Indicadores Criados

  - Receita Líquida
  
  - Despesa Total
  
  - Lucro Operacional
  
  - Margem Operacional (%)
  
  - DSO = media de dias para receber
  
  - DPO = media de dias para pagar

## 4️⃣ IMPACTO
📈 Benefícios para o Negócio

✔ Visibilidade clara da margem real

✔ Identificação de centros de custo críticos

✔ Melhoria potencial na gestão de capital de giro

✔ Redução de risco financeiro

✔ Decisões baseadas em dados

💰 Impacto Estratégico Estimado

- Redução de 3–8% em custos operacionais críticos

- Melhoria de 5–10 pontos percentuais na margem

- Aumento da previsibilidade de caixa

- Melhor planejamento financeiro
